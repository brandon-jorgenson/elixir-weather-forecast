defmodule WeatherForecastTest do
  ...
  test "failed_average_max_temp" do
    with_mock HTTPoison,
      ...
      assert WeatherForecast.get_average_max_temp("SLC", 1) == "404 Error"
    end
  end
  ...
  test "successful_end_to_end" do
    ...
    with_mock HTTPoison,
      ...
      assert WeatherForecast.get_average_max_temp_for_locations() == :ok
    end
  end

  test "failed_end_to_end" do
    # the console should print out the 404 error
    with_mock HTTPoison,
      get: fn _url, [], follow_redirect: true ->
        {:ok, %HTTPoison.Response{body: "{\"detail\":\"Not found.\"}", status_code: 404}}
      end do
